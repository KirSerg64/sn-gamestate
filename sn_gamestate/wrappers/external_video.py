        ...
        if self.video_path.is_dir():
            ...
            for i, video_path in enumerate(tqdm(list(self.video_path.iterdir()))):
                if not mimetypes.guess_type(video_path)[0].startswith('video'):
                    continue
                nframes,w,h = self.get_frame_count(video_path)
                video_name = video_path.stem
                video_id = video_name
                ...
                video_metadata.append({"id": video_name, "name": video_name, "width": w, "height": h})
            image_metadata = pd.DataFrame(image_metadata)
            video_metadata = pd.DataFrame(video_metadata, index=video_names)
        else:
            nframes, w, h = self.get_frame_count(self.video_path)
            video_id = 0
            ...
            video_metadata = pd.DataFrame([{"id": video_name, "name": video_name, "width": w, "height": h}])
